# 2023.3: Introduction to Modeling in Forge
...
This week will be a sort of "whirlwind tour" of Forge. We'll cover more in future classes; you can also access the [Forge documentation](https://csci1710.github.io/forge-documentation/). We just moved the documentation to a new platform this semester, so bear with us as we update it.

### Forge Updates

We'll be updating Forge regularly; expect an update roughly every week. Sometimes we may update more often to resolve issues, etc. Updates will be announced on EdStem, and each update will have a new version number. **Please keep Forge updated** and include your version number in questions or bug reports.
...
<!-- TODO: Forge needs #lang froglet added and explained -->
...
The first line of any Forge model will be a `#lang` line, which says which sub-language the file is. We'll start with the Froglet language for now. Everything you learn in this language will apply in other Forge languages, so I'll use "Forge" interchangably.
...
<!-- this shows the play button if we say it is rust...
  related to the removing language-... ?
 -->
<!-- ```forge,editable -->
```forge,editable
#lang froglet
...
* Forge integers aren't true mathematical integers, but are bounded by a bitwidth we give whenever we run the tool. So we need to be careful here. We want a classical 3-by-3 board, not a board where row `-5`, column `-1` is a valid location. 
...
For now, we'll use the "Table" visualization---we'll improve this next time. This instance contains a single board, and it has 9 entries. Player `O` has moved in all of them (the `$0` suffix of `O$0` in the display is an artifact of how Forge's engine works). It's worth noticing two things:
* This board doesn't look quite right: player `O` occupies all the squares. We might ask: has player `O` been cheating? But the fact is that this board _satisfies the constraints we have written so far_. Forge produces it simply because our model isn't yet restrictive enough, and for no other reason.
